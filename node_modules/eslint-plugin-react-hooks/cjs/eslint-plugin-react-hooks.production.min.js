...
a.parent.key}
function xa(a){function d(){return{isUsed:!1,isSatisfiedRecursively:!1,isSubtreeUsed:!1,children:new Map}}function f(k,n){n=n.split(".");n=P(n);var D;try{for(n.s();!(D=n.n()).done;){var F=D.value,G=k.children.get(F);G||(G=d(),k.children.set(F,G));k=G}}catch(I){n.e(I)}finally{n.f()}return k}function p(k,n,D){n=n.split(".");n=P(n);var F;try{for(n.s();!(F=n.n()).done;){var G=k.children.get(F.value);if(!G)break;D(G);k=G}}catch(I){n.e(I)}finally{n.f()}}function t(k,n,D,F){k.children.forEach(function(G,I){var S=
F(I);G.isSatisfiedRecursively?G.isSubtreeUsed&&D.add(S):G.isUsed?n.add(S):t(G,n,D,function(Q){return S+"."+Q})})}var H=a.dependencies,r=a.declaredDependencies,h=a.stableDependencies,x=a.externalDependencies,L=a.isEffect,C=d();H.forEach(function(k,n){f(C,n).isUsed=!0;p(C,n,function(D){D.isSubtreeUsed=!0})});r.forEach(function(k){f(C,k.key).isSatisfiedRecursively=!0});h.forEach(function(k){f(C,k).isSatisfiedRecursively=!0});a=new Set;var R=new Set;t(C,a,R,function(k){return k});var m=[],q=new Set,y=
new Set;r.forEach(function(k){k=k.key;R.has(k)?-1===m.indexOf(k)?m.push(k):y.add(k):!L||k.endsWith(".current")||x.has(k)?q.add(k):-1===m.indexOf(k)&&m.push(k)});a.forEach(function(k){m.push(k)});return{suggestedDependencies:m,unnecessaryDependencies:q,duplicateDependencies:y,missingDependencies:a}}
...
u);return u}function R(l){var A=R.cache,z=A.get(l.id);if(null===z)return Infinity;if(void 0!==z)return z;A.set(l.id,null);if(0===l.prevSegments.length)z=1;else{z=Infinity;var u=P(l.prevSegments),E;try{for(u.s();!(E=u.n()).done;){var B=R(E.value);B<z&&(z=B)}}catch(M){u.e(M)}finally{u.f()}z+=1}A.set(l.id,z);return z}var m=p.pop();if(0!==m.size){var q=new Set;L.cache=new Map;C.cache=new Map;R.cache=new Map;var y=C(h.initialSegment),k=wa(x),n=va(x),D=k?oa(k)||na(k):ta(x)||ua(x),F=Infinity,G=P(h.finalSegments),
I;try{for(G.s();!(I=G.n()).done;){var S=I.value;if(S.reachable){var Q=R(S);Q<F&&(F=Q)}}}catch(l){G.e(l)}finally{G.f()}m=P(m);var N;try{for(m.s();!(N=m.n()).done;){var ca=N.value,T=ca[0],qa=ca[1];if(T.reachable){var ia=0===T.nextSegments.length?F<=R(T):F<R(T),O=L(T)*C(T),fa=q.has(T.id),X=P(qa),Z;try{for(X.s();!(Z=X.n()).done;){var J=Z.value;fa&&a.report({node:J,message:'React Hook "'+a.getSource(J)+'" may be executed more than once. Possibly because it is called in a loop. React Hooks must be called in the exact same order in every component render.'});
...
x.type){var ja='React Hook "'+a.getSource(J)+'" cannot be called at the top level. React Hooks must be called in a React function component or a custom React Hook function.';a.report({node:J,message:ja})}else if(n){var da='React Hook "'+a.getSource(J)+'" cannot be called inside a callback. React Hooks must be called in a React function component or a custom React Hook function.';a.report({node:J,message:da})}}}catch(l){X.e(l)}finally{X.f()}}}}catch(l){m.e(l)}finally{m.f()}}},r.CallExpression=function(h){if(na(h.callee)){var x=
...
enableDangerousAutofixThisMayCauseInfiniteLoops:{type:"boolean"}}}]},create:function(a){function d(m){t&&Array.isArray(m.suggest)&&0<m.suggest.length&&(m.fix=m.suggest[0].fix);a.report(m)}function f(m,q){return function(y){if(q.has(y))return q.get(y);var k=m(y);q.set(y,k);return k}}function p(m,q,y,k,n){function D(b){var e=P(b.references),c;try{for(e.s();!(c=e.n()).done;){var g=c.value;if(g.resolved&&S.has(g.resolved.scope)){var w=Ga(m,g.identifier),v=Aa(w),ba=Y(v,fa),ea;if(ea=n&&"Identifier"===v.type&&
("MemberExpression"===v.parent.type||"OptionalMemberExpression"===v.parent.type)&&!v.parent.computed&&"Identifier"===v.parent.property.type&&"current"===v.parent.property.name){for(var W=g.from,ka=!1;W.block!==m;)"function"===W.type&&(ka=null!=W.block.parent&&"ReturnStatement"===W.block.parent.type),W=W.upper;ea=ka}ea&&ia.set(ba,{reference:g,dependencyNode:v});if("TSTypeQuery"!==v.parent.type&&"TSTypeReference"!==v.parent.type){var la=g.resolved.defs[0];if(null!=la&&(null==la.node||la.node.init!==
m.parent)&&"TypeParameter"!==la.type)if(O.has(ba))O.get(ba).references.push(g);else{var Da=g.resolved,Ja=T(Da)||qa(Da);O.set(ba,{isStable:Ja,references:[g]})}}}}}catch(ra){e.e(ra)}finally{e.f()}b=P(b.childScopes);var Ea;try{for(b.s();!(Ea=b.n()).done;)D(Ea.value)}catch(ra){b.e(ra)}finally{b.f()}}function F(b){b=b.split(".");for(var e="",c=0;c<b.length;c++){if(0!==c){var g=b.slice(0,c+1).join(".");g=!0===fa.get(g);e+=g?"?.":"."}e+=b[c]}return e}function G(b,e,c,g){return 0===b.size?null:(1<b.size?
"":e+" ")+c+" "+(1<b.size?"dependencies":"dependency")+": "+Ia(Array.from(b).sort().map(function(w){return"'"+F(w)+"'"}))+(". Either "+g+" "+(1<b.size?"them":"it")+" or remove the dependency array.")}n&&m.async&&d({node:m,message:"Effect callbacks are synchronous to prevent race conditions. Put the async function inside:\n\nuseEffect(() => {\n  async function fetchData() {\n    // You can await here\n    const response = await MyAPI.getData(someId);\n    // ...\n  }\n  fetchData();\n}, [someId]); // Or [] if effect doesn't need props or state\n\nLearn more about data fetching with Hooks: https://reactjs.org/link/hooks-data-fetching"});
for(var I=r.acquire(m),S=new Set,Q=null,N=I.upper;N;){S.add(N);if("function"===N.type)break;N=N.upper}if(N){Q=N;var ca=Array.isArray,T=f(function(b){if(!ca(b.defs))return!1;var e=b.defs[0];if(null==e||"VariableDeclarator"!==e.node.type)return!1;var c=e.node.init;if(null==c)return!1;for(;"TSAsExpression"===c.type||"AsExpression"===c.type;)c=c.expression;var g=e.node.parent;if(null==g&&(Ga(Q.block,e.node.id),g=e.node.parent,null==g))return!1;if("const"===g.kind&&"Literal"===c.type&&("string"===typeof c.value||
...
isEffect:n});N=K.unnecessaryDependencies;var U=K.missingDependencies,ja=K.duplicateDependencies,da=K.suggestedDependencies;if(0===ja.size+U.size+N.size)ya({declaredDependencies:J,declaredDependenciesNode:q,componentScope:Q,scope:I}).forEach(function(b){var e=b.construction,c=b.isUsedOutsideOfHook;b=b.depType;var g="function"===b?"useCallback":"useMemo",w="function"===b?"definition":"initialization",v="wrap the "+w+" of '"+e.name.name+"' in its own "+g+"() Hook.";v="The '"+e.name.name+"' "+b+" "+("conditional"===
...
da.map(F).join(", ")+"]")}}]})}}else{var M=null;O.forEach(function(b,e){M||b.references.forEach(function(c){if(!M&&h.has(c.identifier)){for(c=c.from;"function"!==c.type;)c=c.upper;c.block===m&&(M=e)}})});if(M){var sa=xa({dependencies:O,declaredDependencies:[],stableDependencies:Z,externalDependencies:new Set,isEffect:!0}).suggestedDependencies;d({node:y,message:"React Hook "+k+" contains a call to '"+M+"'. Without a list of dependencies, this can lead to an infinite chain of updates. To fix this, pass ["+
sa.join(", ")+("] as a second argument to the "+k+" Hook."),suggest:[{desc:"Add dependencies array: ["+sa.join(", ")+"]",fix:function(b){return b.insertTextAfter(m,", ["+sa.join(", ")+"]")}}]})}}}}var t=a.options&&a.options[0]&&a.options[0].enableDangerousAutofixThisMayCauseInfiniteLoops||!1,H={additionalHooks:a.options&&a.options[0]&&a.options[0].additionalHooks?new RegExp(a.options[0].additionalHooks):void 0,enableDangerousAutofixThisMayCauseInfiniteLoops:t},r=a.getSourceCode().scopeManager,h=new WeakMap,
x=new WeakSet,L=new WeakMap,C=new WeakMap,R=new WeakSet;return{CallExpression:function(m){var q=Fa(m.callee,H);if(-1!==q){var y=m.arguments[q],k=m.callee,n=Ca(k).name;m=m.arguments[q+1];var D=!m||"Identifier"===m.type&&"undefined"===m.name?void 0:m;m=/Effect($|[^a-z])/g.test(n);if(y)if(D||m){switch(y.type){case "FunctionExpression":case "ArrowFunctionExpression":p(y,D,k,n,m);return;case "Identifier":if(!D||D.elements&&D.elements.some(function(F){return F&&"Identifier"===F.type&&F.name===y.name}))return;
q=a.getScope().set.get(y.name);if(null==q||null==q.defs)return;q=q.defs[0];if(!q||!q.node)break;if("Variable"!==q.type&&"FunctionName"!==q.type)break;switch(q.node.type){case "FunctionDeclaration":p(q.node,D,k,n,m);return;case "VariableDeclarator":if(q=q.node.init)switch(q.type){case "ArrowFunctionExpression":case "FunctionExpression":p(q,D,k,n,m);return}}break;default:d({node:k,message:"React Hook "+n+" received a function whose dependencies are unknown. Pass an inline function instead."});return}d({node:k,
message:"React Hook "+n+" has a missing dependency: '"+y.name+"'. Either include it or remove the dependency array.",suggest:[{desc:"Update the dependencies array to be: ["+y.name+"]",fix:function(F){return F.replaceText(D,"["+y.name+"]")}}]})}else"useMemo"!==n&&"useCallback"!==n||d({node:k,message:"React Hook "+n+" does nothing when called with only one argument. Did you forget to pass an array of dependencies?"});else d({node:k,message:"React Hook "+n+" requires an effect callback. Did you forget to pass a callback to the hook?"})}}}}}};
